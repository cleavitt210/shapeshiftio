...
def mkurl(base, *parts):
    return urljoin(base, os.path.join(*parts))


def rate(pair=''):
    """
    Gets the current rate offered by Shapeshift. This is an estimate because 
    the rate can occasionally change rapidly depending on the markets. The rate 
    is also a 'use-able' rate not a direct market rate. Meaning multiplying 
    your input coin amount times the rate should give you a close approximation 
    of what will be sent out. This rate does not include the transaction 
...
def limit(pair):
    ...
    this limit will be sent to the return address if one was entered, otherwise 
    the user will need to contact ShapeShift support to retrieve their coins. 
...
def market_info(pair):
    ...
    The pair is not required and if not specified will return an array of all 
    market infos.
    ...
    """
    url = mkurl(BASE_URL, 'marketinfo', pair)
    return requests.get(url).json()


def recent_tx(max_results=5):
    """
    ...
    """
    url = mkurl(BASE_URL, 'recenttx', str(max_results))
    return requests.get(url).json()


def tx_status(address):
    """
This returns the status of the most recent deposit transaction to the address.
    ...
    """
    url = mkurl(BASE_URL, 'txStat', address)
    return requests.get(url).json()
...
def time_remaining(address):
    ...
    it will include the "?dt=destTagNUM" appended on the end, and you will need 
    to use the URIEncodeComponent() function on the address before sending it 
    in as a param, to get a successful response.
    ...
    """
    url = mkurl(BASE_URL, 'timeremaining', address)
    return requests.get(url).json()


def coin_list():
    """
    Allows anyone to get a list of all the currencies that Shapeshift currently
    supports at any given time. The list will include the name, symbol,
    availability status, and an icon link for each.
    ...
    become temporarily unavailable during updates or unexpected service issues.
    """
    url = mkurl(BASE_URL, 'getcoins')
    return requests.get(url).json()


def tx_by_apikey(apiKey):
    """
    ...
    """
    url = mkurl(BASE_URL, 'txbyapikey', apiKey)
    return requests.get(url).json()


def tx_by_address(apiKey, address):
    """
    ...
    and will only return transactions that were sent to output address AND were
    created using / linked to the affiliate's PUBLIC KEY. Please note that if
    the address is a ripple address and it includes the "?dt=destTagNUM"
    appended on the end, you will need to use the URIEncodeComponent() function
    on the address before sending it in as a param, to get a successful
    response.
    ...
    The status can be "received", "pending", "verifying_send", "sent_exact",
    "exchanged". "sent_exact" is the same as "exchanged", for all intensive
    purposes, meaning the shift completed, funds were sent in and out and there
    was no error.
    """
    url = mkurl(BASE_URL, 'txbyaddress', address, apiKey)
    return requests.get(url).json()
...
def validate_address(self, address, coin):
    ...
    isValid will either be true or false. If isvalid returns false, an error
    parameter will be present and will contain a descriptive error message.
    """
    url = mkurl(BASE_URL, 'validateAddress', address, coin)
    return requests.get(url).json()


def shift(to_addr, pair, return_addr):
    """
    This is the primary data input into ShapeShift.
        ...
        """
    payload = {'withdrawal'   : to_addr,
               'pair'         : pair,
               'returnAddress': return_addr}
    url = mkurl(BASE_URL, 'shift')
    return requests.post(url, payload).json()


def recipt(email, transaction_id):
    """
    ...
    payload = {'email': email,
               'txid' : transaction_id}
    url = mkurl(BASE_URL, 'mail')
    return requests.post(url, payload).json()


def send_amount(amount, to_addr, pair, return_addr):
    """
    ...
    url = mkurl(BASE_URL, 'sendamount')
    return requests.post(url, payload).json()


def cancel_pending(deposit_addr):
    """
    This call allows you to request for canceling a pending transaction by the
    deposit address. If there is fund sent to the deposit address, this pending
    transaction cannot be canceled.
        ...
        Error Output:
         ...
         {  error  : {errorMessage}  }
    """
    payload = {'address': deposit_addr}
    url = mkurl(BASE_URL, 'cancelpending')
    return requests.post(url, payload).json()
